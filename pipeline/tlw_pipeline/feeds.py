...
def atom(title: str, feed_id: str, items: list[dict], updated: str | None, site: str = SITE) -> str:
    out = ['<?xml version="1.0" encoding="utf-8"?>', '<feed xmlns="http://www.w3.org/2005/Atom">',
           f"<title>{escape(title)}</title>", f"<id>{escape(site)}/{escape(feed_id)}</id>",
           f'<link rel="alternate" href="{escape(site)}/#/{escape(feed_id)}"/>',
           f"<updated>{updated or '1970-01-01'}T00:00:00Z</updated>",
           "<author><name>Thai Legal Watch · data: OpenLawData</name></author>"]
    for it in items:
        link = f"{site}/#/doc/{it['id']}"
